...
template<typename T>
std::string demangled_name(const T &obj) {
    int status;
...
public:
    TemplateLogger(const char invoker[], const char file[], unsigned int line,
                   long int tid, const char *message, ...) {
        this->tid = tid;
        this->line = line;
        ...
        if (current_log_level == 1) {
            adapter.writeOpening(current_time_in_millis(),
                                 this->invoker, this->file, this->line,
                                 message, argp);
        } else {
            adapter.printFormatted(current_time_in_millis(),
                                   this->invoker, this->file, this->line,
    ...
    ~TemplateLogger() {
        ...
        if (current_log_level == 1) {
            adapter.writeEpilogue(
                static_cast<unsigned long>(current_time_in_millis()));
        }
        current_log_level--;
    }
...
template<typename T>
inline thread_local int TemplateLogger<T>::current_log_level = 0;
